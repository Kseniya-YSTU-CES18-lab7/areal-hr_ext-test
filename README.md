...
#### Исправленные ошибки (5,6 недели)
- Авторизация: внедрена сессионная аутентификация (SessionGuard), все эндпоинты защищены
- Сотрудники ↔ отделы: добавлена связь, фильтрация и выбор отдела работают
- Уволенные сотрудники: реализован просмотр, восстановление, блокировка редактирования
- Увольнение через кадровую операцию: операция реально меняет статус сотрудника
- История изменений: теперь сохраняется реальный userId, а не заглушка
- Поиск по ФИО: работает по всем полям (логин, фамилия, имя, отчество)
